...
answer = np.argmax(get_mat(solution[0],'A'),2)
result  = []
for ii in sorted(models):
    answer = np.argmax(get_mat(solution[0],'A'),2)
    if 'Adamiak' in ii: #first resi no 'P atom'
        answer = answer[1:,1:]
    mat_model1 = np.argmax(get_mat(ii,'B'),2)
    mat_model2 = np.argmax(get_mat(ii,'A'),2)
    ...
    for i in range(0,answer.shape[0]):
        for j in range(i+1,answer.shape[0]):
            if answer[i,j] == mat_model[i,j]:
                ...
                score[answer[i,j]] += [0,]
    print ii,i,np.mean([np.mean(score[0]),np.mean(score[1]),np.mean(score[2])])

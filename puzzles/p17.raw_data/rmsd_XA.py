...
for num in sorted([x for x in os.listdir('.') if ('.pdb' in x and 'solution' not in x)]):
    ...
    seqP = ''
    for line in f1:
        if  'ATOM' in line and line[17:20].strip() in ['ADE','URI','GUA','CYT','A','U','G','C'] and line[12:16].strip() == "C1'":
    ...
    seqQ = ''
    for line in f2:
        if  'ATOM' in line and line[17:20].strip() in ['ADE','URI','GUA','CYT','A','U','G','C'] \
